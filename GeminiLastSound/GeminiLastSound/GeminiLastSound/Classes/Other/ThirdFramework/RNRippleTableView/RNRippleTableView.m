...
- (void)generateHeightAndOffsetData {
    BOOL checkForHeightEachRow = [self.rippleDelegate respondsToSelector:@selector(heightForViewInTableView:atIndex:)];
    ...
    @autoreleasepool {
        for (NSInteger i = 0; i < numberOfRow; i++) {
            ...
            rowObject.index = i;
            rowObject.height = checkForHeightEachRow ? [self.rippleDelegate heightForViewInTableView:self atIndex:i] : kRNRippleViewCellDefaultHeight;
...
- (void)handleTapGesture:(UITapGestureRecognizer *)recognizer {
    if (recognizer == self.tapGesture && [self.rippleDelegate respondsToSelector:@selector(tableView:didSelectView:atIndex:)]) {
        ...
        if (view) {
            ...
            if (rowObject) {
                ...
                [self.rippleDelegate tableView:self didSelectView:view atIndex:rowObject.index];
            }
        }
    }
}
